...
void* robot_move(void* arg)
{
	...
	double x=-0.005;
	SETPOSE(j0,x,-5*un,z+0.05,k,p,s);
	...
	SETSPEED(sp,0.05,0.020);
	SETSPEED(sp1,0.05,0.02);//0.3
	SETSPEED(sp2,0.05,0.02);//0.5
...
int MainModule()
{
	...
	//MultiMoveA(j31,sp,NULL3,2);

	//MultiMoveJ(j0,sp,NULL,toolgg1,wobjgg1,1);
	//MultiMoveL(j1,sp,NULL,toolgg1,wobjgg1,1);


	IMPORTJOINT(ji1);
	MultiMoveA(ji1,sp,NULL3,1);

	MultiMoveJ(j0,sp,NULL,toolgg0,wobjgg0,0);
	MultiMoveL(j1,sp,NULL,toolgg0,wobjgg0,0);
...
*/
	...
	for (i=0;i<5;i++)
	{
		MultiMoveL(pt1,sp,NULL,NULL,NULL,1);
                MultiMoveL(pt0,sp,NULL,NULL,NULL,1);
	}
