 ...
 *
 * @param centre 球心（单位：m）
 * @param radius 球半径（单位：m）
 ...
 *
 * @param centre 立方体角点（单位：m）
 * @param length centre所在长边的另一个角点（单位：m）
 * @param width centre所在宽边的另一个角点（单位：m）
 * @param high centre所在高边的另一个角点（单位：m）
 * @param property 区域性质，_constraint_notallow:完全不允许区域; _constraint_slowallow:不允许区域，但允许缓慢移动;_constraint_allow:完全允许区域，其他区域完全不允许;_constraint_allowslow:完全允许区域，其他区域允许缓慢移动;
 * @param rtool 末端工具（球体描述坐标系，工具不在机器人上有效）
 * @param rwobj 工件坐标系（球体描述坐标系，工件不在机器人上有效）
 * @param name 指定几何体名字
 ...
 *
 * @param centreb 下底面圆形点（单位：m）
 * @param centreh  上底面圆形点（单位：m）
 * @param inner_radius 内半径（单位：m）
 * @param external_radius 外半径（单位：m）
 * @param property 区域性质，_constraint_notallow:完全不允许区域; _constraint_slowallow:不允许区域，但允许缓慢移动;_constraint_allow:完全允许区域，其他区域完全不允许;_constraint_allowslow:完全允许区域，其他区域允许缓慢移动;
 * @param rtool 末端工具（球体描述坐标系，工具不在机器人上有效）
