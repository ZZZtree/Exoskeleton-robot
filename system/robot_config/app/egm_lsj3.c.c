...
void* rec_function(void* arg) {
        ...
        if (top < (NumArray-2)) 
        {
            ...
            {  
                if(receivedBytes > 0)
                {
                    ...
                    bufferF[top][1] = -1 * camera_data[1] + pinit.xyz[1];
                    bufferF[top][2] = camera_data[2] + pinit.xyz[2];
                    bufferF[top][3] = camera_data[3] + pinit2.xyz[0];
                    bufferF[top][4] = -1 * camera_data[4] + pinit2.xyz[1];
 ...
 void* rec_function2(void* arg) {
    ...
    while (robot_ok())//寰幆鎿嶄綔
    {
        ...
        SETPOSE(pc,bufferF[0][3],bufferF[0][4],bufferF[0][5],pinit2.kps[0],pinit2.kps[1],pinit2.kps[2]);//璁剧疆鐩爣鏁版嵁
        

        /****夹爪控制****/
        if(bufferF[0][8] == 0.0 && GripLastR != 0.0)      //close
        {
            SetDo(17,1);
            Rsleep(50);
            SetDo(17,0);
        }
        else if(bufferF[0][8] == 1.0 && GripLastR != 1.0)      //open
        {
            SetDo(16,1);
            Rsleep(50);
            SetDo(16,0);
        ...
        //     multi_moveL(pc,vt,NULL,NULL,NULL,1);
        // else
        //     multi_moveL(pc,vt,NULL,NULL,NULL,1);

        GripLastR = bufferF[0][8];    
...
void* rec_function_init(void* arg) {
    SETJOINT7(path1_initR, 1.998, 1.0624, -0.269, 1.239, 0.223, 0.733, 0.5742);
...
int MainModule()
{
    SETJOINT7(path1_initL, -1.117, 0.9774, -0.3142, 1.326, 0.3665, 0.7505, 0.5760);
    IMPORTWOBJ(wobj0);
    SETZONE(zone,0.1);
    ...
    while (robot_ok())//寰幆鎿嶄綔
    {
        ...
        /****夹爪控制****/
        if(bufferF[0][7] == 0.0 && GripLastL != 0.0)      //close
        {
            SetDo(1,1);
            Rsleep(50);
            SetDo(1,0);
        }
        else if(bufferF[0][7] == 1.0 && GripLastL != 1.0)      //open
        {
            SetDo(0,1);
            Rsleep(50);
            SetDo(0,0);
