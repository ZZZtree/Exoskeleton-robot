...
void* robot_corr(void* arg)
{
	...
	double R[3][3];
	rpy2r(pinit.kps, R);
	double Rinit[3][3]={{R[0][0],R[1][0],R[2][0]},
                         {R[0][1],R[1][1],R[2][1]},
                         {R[0][2],R[1][2],R[2][2]}};	
	while (robot_ok())
	{
		...
		target[0]=pc.xyz[0]-pinit.xyz[0];target[1]=pc.xyz[1]-pinit.xyz[1];target[2]=pc.xyz[2]-pinit.xyz[2];
		rpy2r(pc.kps, R);
		double tmp[3][3];
		rxr(R,Rinit,tmp);
		r2rpy(tmp,&(target[3]));
...
void* robot_move(void* arg)
{
	...
	SETSPEED(sp,0.05,0.10);
	SETSPEED(sp1,0.05,0.1);//0.3
	SETSPEED(sp2,0.05,0.1);//0.5
	...
	MultiMoveJ(j0,sp,NULL,to,wo,robot_index);
...
int MainModule()
{
	...
	IMPORTJOINT(ji0);
        MultiMoveA(ji1,sp,NULL3,0);

	IMPORTJOINT(ji1);
	MultiMoveA(ji1,sp,NULL3,1);

        IMPORTJOINT(ji2);
        MultiMoveA(ji2,sp,NULL3,1);

	

	int ret=EGMCreate(2,toolgg0, wobjgg0,0);
	if (0!=ret)
	{
	...
	int ret=EGMCreate(2,toolgg1, wobjgg1,1);
	if (0!=ret)
	{
	...
	Rsleep(500);
	int ret=ThreadCreat(robot_move,&robot0,"robot0_move",1 );
	if (0!=ret)
	{
		printf("robot 0 ThreadCreat failure\n");
		return 0;
	...
	while (robot_ok())
	{
		MultiMoveL(p32,sp,zone,toolgg2,wobjgg2,2);
		MultiMoveL(p33,sp,zone,toolgg2,wobjgg2,2);
		MultiMoveL(p31,sp,NULL,toolgg2,wobjgg2,2);
	}
