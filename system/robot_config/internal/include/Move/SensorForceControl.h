 ...
 * @param is_open_sensor 是否创建传感器，1：是；0：否，如果为0，传感器必须已经被创建
 * @return int 成功返回0，失败返回其他
 */
extern int SFCSetForceSensorName(const char* name, const char* sensor_name, int is_open_sensor);
...
/**
 * @brief 释放数据资源，执行后如需使用力控，需要重新执行SFCInit()
 *
 * @param name 创建时指定的名称
 * @return int 成功返回0，失败返回其他
 */
extern int SFCClose(const char* name);
