...
int _deal(int fd)
{
        ...
        }
        {
            ...
            } else if ("6"==std::string(buf))
            {
                ...
                MultiMoveA(R0_j3,R0_v50,NULL3,0);//竖拇指
                MultiMoveA(R1_j3,R0_v50,NULL3,1);
                MultiMoveAdd(A0_jhome, R0_v50, NULL3, 0);
                usleep(10000);
                MoveSyncStart();
                
            // } else if (1==js.type && 7==js.number)
            } else if ("5"==std::string(buf))
            {
                ROBOT_INFO("move\n");
                DeviceStop();
                isstop.store(true);
...
void PluginMain()
{
    ROBOT_INFO("PluginMain:SUPCONPlugin init\n");
    ...
    t=std::thread([](){
        isshow.store(false);
        isstop.store(false);
        TCPConcurrentServer(NULL, 8888, _deal);
    });
